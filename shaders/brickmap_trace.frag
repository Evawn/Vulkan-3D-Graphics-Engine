...
layout(set = 0, binding = 0) uniform sampler3D volume_sampler;

layout(std430, set = 0, binding = 1) readonly buffer BrickmapBuffer {
	uint bm_volume_size;
	uint bm_brick_size;
	uint bm_grid_dim;
	uint bm_brick_count;
	uint bm_top_grid[4096];
	uint bm_brick_bitmasks[];
};
...
// Test if a voxel is occupied in a brick's bitmask
bool brickVoxelOccupied(uint brick_index, ivec3 local) {
	int bit_idx = local.z * 64 + local.y * 8 + local.x;
	uint word = bm_brick_bitmasks[brick_index * 16 + bit_idx / 32];
	return (word & (1u << (bit_idx % 32))) != 0;
}
	...
	for (int outer_step = 0; outer_step < pc.maxIterations; outer_step++) {
		...
		if (brick_index != 0xFFFFFFFFu) {
			...
			for (int inner_step = 0; inner_step < max_inner_steps; inner_step++) {
				...
				// Test occupancy via bitmask
				if (brickVoxelOccupied(brick_index, local_coord)) {
					// HIT — shade by face direction
					vec4 color = vec4(vec3(1.0) - vec3(inner_face) * 0.1, 1.0);
					if (pc.debugColor != 0) {
